...
// Y CADA MOTOR TAMBIÉN TIENE SU PROPIO VOCABULARIO PARA EL VEREDICTO (16-sep, segunda pasada). La primera
// versión reconocía WIN/LOSS/VOID/PUSH y `futbol-derivadas` escribe `won`, `lost`, `push`, `half_won` y
// `half_lost` — las dos últimas porque un cuarto asiático puede ganar la mitad de la apuesta. Resultado: de
// sus 6.611 liquidadas el módulo reconocía 369, y la tasa de ausencia salía **83 %** cuando la real es
// **21,5 %**. Un cuádruple, y en la dirección alarmante.
//
// Es exactamente el fallo que este módulo existe para cazar, cometido por el módulo. Merece quedar escrito:
// un accesor que no habla el idioma del motor no devuelve «no sé», devuelve un número con toda la pinta de
// ser una medición. Por eso ahora la lista es explícita, larga y con el motor de cada término al lado.
const OBSERVADAS = new Set([
  'WIN', 'LOSS', 'VOID', 'PUSH',              // esports, tenis, TT, dardos, NFL, hoops, clubes
  'WON', 'LOST',                              // futbol-derivadas
  'HALF_WON', 'HALF_LOST',                    // futbol-derivadas: cuartos asiáticos, media apuesta
]);

function examinaLibro(picks, opciones = {}) {
  return examina(picks, {
    observada: (p) => OBSERVADAS.has(VEREDICTO(p)),

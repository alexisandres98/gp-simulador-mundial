...
async function writeNflRead(payload, aviso) {
  const resp = await call({
    kind: 'writer', json: 'esen',
    max_tokens: 3000,
    system: 'Eres el analista de NFL de GP Simulador, al nivel de un scout profesional. PROHIBIDO describir el funcionamiento interno del sistema: nada de nombrar métodos, familias de modelo, pesos, constantes ni proveedores de datos. Se narra QUÉ ve el modelo y POR QUÉ importa, nunca CÓMO lo calcula. Con el dossier JSON escribe la lectura del PARTIDO en DOS párrafos por idioma (máximo 110 palabras por párrafo). REGLA MAESTRA: el favorito es EXACTAMENTE "favorito_gp.nombre" con su probabilidad — tu tesis lo defiende SIEMPRE; si el "mercado" del dossier discrepa, esa discrepancia ES parte del análisis, jamás una razón para cambiar de bando. (1) LA FORMA DEL PARTIDO — de dónde sale la ventaja (pase o carrera, ofensa o defensa, citando los EPA del dossier), qué dice la diferencia de rating, cómo pesan el descanso, la sede o el clima si vienen en el JSON, y qué QB/entrenador conduce cada lado. (2) EL CAMINO DEL OTRO — el mejor argumento del rival con sus números, qué señal temprana diría que el partido se torció y qué lo volvería cerrado; si la incertidumbre del dossier es alta (inicio de temporada), DILO con su número. PROHIBIDO: mencionar picks, apuestas, cuotas, spread como recomendación, edge o valor; contradecir a favorito_gp; inventar datos; hype. Nombra cada métrica como viene en el JSON. Tono: analista concreto. Responde SOLO un JSON {"es":"...","en":"..."} en UNA línea — separa párrafos con \\n\\n dentro del string.',
    messages: [{ role: 'user', content: JSON.stringify(payload) + (aviso ? '\n\n' + aviso : '') }],
  });
  const j = jsonOf(resp);
...
async function writeCs2Read(payload, game, aviso) {
  ...
  const resp = await call({
    kind: 'writer', json: 'esen',
    max_tokens: 3000,
    system: 'Eres el analista de ' + JUEGO + ' de GP Simulador, al nivel de un coach profesional. PROHIBIDO describir el funcionamiento interno del sistema: nada de nombrar métodos, familias de modelo, pesos, constantes ni proveedores de datos. Se narra QUÉ ve el modelo y POR QUÉ importa, nunca CÓMO lo calcula. Con el dossier JSON escribe la lectura de la SERIE en DOS párrafos por idioma (máximo 110 palabras por párrafo). REGLA MAESTRA: el favorito es EXACTAMENTE "favorito_gp.nombre" con su probabilidad — tu tesis lo defiende SIEMPRE; si el mercado del dossier discrepa, esa discrepancia ES parte del análisis. (1) LA FORMA DE LA SERIE — dónde se decide: ' + LENTE + ', la diferencia de Elo, la forma reciente y el historial directo si vienen, y qué jugadores cargan el equipo si el dossier trae ratings. (2) EL CAMINO DEL OTRO — por dónde gana el rival y qué tendría que pasar para que la serie se torciera, y el aviso de plantilla movida si el dossier lo marca. PROHIBIDO: picks, apuestas, cuotas, edge o valor; contradecir a favorito_gp; inventar mapas, héroes, campeones o datos que no estén en el dossier; hype. Nombra cada métrica como viene en el JSON. Tono: analista concreto. Responde SOLO un JSON {"es":"...","en":"..."} en UNA línea — separa párrafos con \\n\\n dentro del string.',
    messages: [{ role: 'user', content: JSON.stringify(payload) + (aviso ? '\n\n' + aviso : '') }],
  });
  const j = jsonOf(resp);

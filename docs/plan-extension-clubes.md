...
| Bracket → Liguilla/playoffs (MX, MLS) | ✔ **prod `62d1a82`**: bracket de playoffs PROYECTADO desde el season sim (siembra por exp_rank + avance por Elo + campeón MC), reusa gx-bk del Mundial, selector `#gx-bcomp`, honesto "proyección/se actualiza cada jornada". Ligas sin fase final = nota; sin temporada (ligamx) = nota |
...
- [x] **F1.3 xG post-partido** ✔ prod `3ca43ff`: clubXgReport agrega el player-history de la liga por equipo (xG/remates/SOT) para el cruce recién jugado → panel mvXg del Mundial; en el cockpit solo si el match del backfill es fresco ±4 días (aparece conforme los backfills se actualizan). También **F1.1c stats del partido** ✔: clubMatchStats (AF statistics: posesión/remates/SOT/córners/faltas/amarillas/xG, memo 60s, live/final) → panel mvStats COMPLETO del Mundial.
...
- [x] **F3.3 Curate multi-liga** ✔ prod `53302ae` — **MISMO pipeline del Mundial**: curate()+settleOne() tal cual, mercado del sweep de clubes (consenso/best con las funciones del scanner), modelo Elo 1X2 + goal engine λ de liga, narrativa compose(). Gates duros: SOLID solo clubs-gate-1 approved; GOALS por familyApproved=goals_backtest approved. db.clubDailyPicks + settle reg90 en clubScoresSync + track record por liga/familia + feed merge admin (chip liga, click al cockpit) + /api/internal/clubs-picks (?dry=1). Verificado dry-run contra prod: 175 mercados, 0 SOLID (gates shadow), 88 GOALS todos bloqueados FAMILY_NOT_APPROVED — nacen solas cuando Premier/Bundesliga/SerieA arranquen en agosto.
- [x] **F3.4 Props de clubes** ✔ prod `7978dc7` — CORNERS/CARDS pipeline COMPLETO (mismo prop-engine del Mundial): props-history por liga (clubs-props-backfill, AF stats; BRA/MLS/ARG/COL/PAR/J1 con 122-218 partidos) → clubPropsFit + backtest LOO (**córners+tarjetas APPROVED en 6 ligas**) → ingesta alternate_totals_corners/cards en el sweep de clubes → curate() con propsRequireApprovedFamily:false (gate viaja) → settlement props-history + AF fallback + VOID 72h. Player props (assists+anytime) ya estaban (a9de21a). **Nacen solas al resetear la cuota de The Odds API (hoy agotada 20k/20k — TODOS los sweeps congelados)**. Tactical profile ganó filas córners/tarjetas (props→fitStyles).

...
// ══ UNA CAPTURA EN VIVO NO ES UN CIERRE (15-sep, hallazgo A11 de la auditoría externa) ═════════════════
// Los motores marcan cada captura con `prepartido` | `in_play` | `desconocido` (ver `implied-engine/closes.js`
// → `estadoCaptura`). Un precio tomado con el partido ya rodando mide otra cosa —el mercado sabe cosas que
// nosotros no sabíamos al entrar— y encima hacia el lado que nos favorece, porque las líneas que se mueven
// rápido son justo las de los partidos que empiezan movidos. Esas filas NO entran a `q_cierre` ni al EV: se
// cuentan aparte (`cierres_in_play`) para que el hueco se vea en vez de desaparecer en una media.
//
// `desconocido` se sigue tratando como hasta hoy: sin inicio fiable no se puede afirmar que la captura fuera
// en vivo, y descartarla por si acaso destruiría el histórico entero, que nació sin esta etiqueta.
function capturaDe(it) {
  if (!it || typeof it !== 'object') return 'desconocido';
  for (const c of ['close_captura', 'cierre_captura', 'captura']) {
    const v = it[c];
    if (typeof v === 'string' && v) return v;
  }
  if (it.close_in_play === true || it.cierre_in_play === true) return 'in_play';
  const last = it.closes && it.closes.last;
  if (last && last.in_play === true) return 'in_play';
  return 'desconocido';
}
const esEnVivo = (it, captura) => (captura || capturaDe)(it) === 'in_play';
...
function evDeFamilia(items, { odds, cierre, cierreContraria, margenLadoPct, evento, costes = 0, captura = null }) {
  ...
  const filas = [], descartados = { sin_cierre: 0, sin_contraria_ni_Q: 0, sin_entrada: 0, cierre_in_play: 0 };
  let aproximados = 0;
  for (const x of items) {
    // el cierre capturado en vivo no se valora (15-sep, A11); se cuenta y se sigue
    if (esEnVivo(x, captura)) { descartados.cierre_in_play++; continue; }
...
function familia(items, { fecha, clv, margenLadoPct = null, nMargen = 0, cuotaMedia = 2, bankroll = null, recorte = 0.10,
  odds = null, cierre = null, gano = null, pModelo = null, cierreContraria = null, evento = null, costes = 0, captura = null } = {}) {
  // PRIMERO SE SEPARAN LAS CAPTURAS EN VIVO (15-sep, A11). Todo lo que consume el CIERRE —la serie de CLV,
  // las semanas, la rodante, `cierreAporta` y el EV— se calcula SOLO sobre las capturas prepartido (y las
  // de inicio desconocido, que son el histórico anterior a la etiqueta). `modeloContraPrecio` se queda con
  // TODAS: no mira el cierre, compara la probabilidad del modelo con la del precio de entrada contra
  // resultados reales, y ahí una captura en vivo no contamina nada.
  const todas = items || [];
  const enVivo = [], usables = [];
  for (const it of todas) (esEnVivo(it, captura) ? enVivo : usables).push(it);
  const vals = usables.map(clv).filter((x) => Number.isFinite(x));
  const s = serie(vals, { recorte });
  // las dos pruebas de método: ¿sirve el CLV aquí? y, si no, ¿acierta el modelo más que el precio?
  const ca = (odds && cierre && gano) ? cierreAporta(usables, { odds, cierre, gano, overPct: margenLadoPct != null ? margenLadoPct * 2 : 0 }) : null;
  const mp = (odds && pModelo && gano) ? modeloContraPrecio(todas, { odds, pModelo, gano }) : null;
  ...
  const ev = evDeFamilia(usables, { odds, cierre, cierreContraria, margenLadoPct, evento, costes, captura });
  ...
  return { ...s, semanas: semanal(usables, { fecha, clv, recorte }), rodante: rodante(usables, { fecha, clv, recorte }),
    // el hueco se publica, no se esconde: cuántas filas quedaron fuera por haberse capturado con el partido
    // ya rodando, y cuántas de las que sí entran llevan la etiqueta puesta (el resto es histórico sin marcar)
    cierres_in_play: enVivo.length,
    cierres_valorables: usables.length,
    cierres_etiquetados: usables.filter((it) => capturaDe(it) === 'prepartido').length,
    ...v };
}

module.exports = { serie, semanal, rodante, veredicto, tamano, familia, evDeFamilia, cierreAporta, modeloContraPrecio, capturaDe, media, sd, tDe, recorta, MIN_N, MIN_T };

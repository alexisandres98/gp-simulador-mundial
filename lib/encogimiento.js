...
function paraFamilia(filas, opciones = {}) {
  ...
  // UN `c` CON EL INTERVALO PEGADO AL CERO NO ESTÁ MEDIDO (16-sep). Salió del primer barrido real:
  // `lol · KILLS_DNB · cloudbet` daba c = 0,45 con una mejora fuera de muestra respetable (0,0325) y su
  // intervalo era **[0, 0,95]** sobre 28 racimos de evento — o sea, toda la rejilla. Publicar «c = 0,45»
  // ahí sería publicar un número que el propio remuestreo dice que no sabe. Se marca, y quien decida
  // aplicarlo tiene que ver la marca antes que el número.
  const indistinguible = !!(ic && Array.isArray(ic.ic) && ic.ic[0] <= 0);
  return { ...aj,
    c_no_distinguible_de_cero: indistinguible,
    aviso_c: indistinguible
      ? `el intervalo de c es [${ic.ic[0]}, ${ic.ic[1]}] sobre ${ic.racimos} racimos de evento: incluye el 0, así que este c NO está medido — es compatible con «el modelo no aporta nada».`
      : null,

...
const ACC = {
  // El mismo cuidado que en `lib/ausencia.js`: cada motor nombra el veredicto a su manera, y
  // `futbol-derivadas` usa `won`/`lost` más `half_won`/`half_lost` para los cuartos asiáticos. Una media
  // apuesta ganada NO es media victoria a efectos de calibración —la pregunta es si el suceso ocurrió— así
  // que `half_won` cuenta como 1 y `half_lost` como 0: en un cuarto, la mitad que resuelve dice qué pasó.
  gano: (p) => { const v = String(p.result_code || p.result || '').toUpperCase();
    if (v === 'WIN' || v === 'WON' || v === 'HALF_WON') return 1;
    if (v === 'LOSS' || v === 'LOST' || v === 'HALF_LOST') return 0;
    return null; },

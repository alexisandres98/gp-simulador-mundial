...
const POR_VEREDICTO = [
  ...
  // REVERTIDA el 21-sep-2026 (orden de Alexis): { cs2 · RONDAS_HANDICAP · pinnacle, ev −3.28, t −9.55, n 412 }.
  // Entró aquí por un veredicto `cerrar` que salía del EV contra el cierre — y en esta familia el cierre NO
  // predice mejor que nuestra entrada (`cierreAporta` t 0,98), así que ese EV no es evidencia: se la estaba
  // condenando con un test que no le aplica. Lo medido con todas sus filas: 381 liquidadas, ROI +13,1 %
  // (t 2,03), CLV +0,92 % contra el cierre de Pinnacle (t 2,76), positiva en las cuatro semanas que
  // produjo. La retirada del 15-sep la sacó del feed el día 13 y dejó viva justo la casa que pierde
  // (Cloudbet, −7,5 %). Bovada (+2,3 %, t 0,42) se queda retirada: no tiene señal que la saque.
...
// EL FEED SIN VEREDICTO (21-sep-2026, orden de Alexis: "quiero que todas esas familias publiquen picks,
// independientemente de si son rentables o no"). La retirada NO desaparece: la lectura medida sigue
// viajando con la fila y la sombra la sigue marcando. Lo que cambia es que trae `publica: true`, y quien
// la consulta deja pasar la pick en vez de tumbarla. Las retiradas POR CONSTRUCCIÓN que son un error de
// cálculo (derivadas_v1, el EV agregado del Boleto GP) NO se publican ni con el interruptor: una cifra
// mal calculada no es una pick sin veredicto, es una cifra falsa.
const FEED = require('./feed');
const PUBLICABLES_POR_CONSTRUCCION = new Set(['lol_kills_hcp_v1', 'ufc_ganador_bruto']);
...
function retirada(deporte, familia, casa = null) {
  ...
  return { motivo: 'veredicto_cerrar', etiqueta: 'control',
    ...
    ...(FEED.sinVeredicto() ? { publica: true } : {}) };
}

function versionRetirada(version) {
  const v = String(version || '');
  if (!POR_CONSTRUCCION[v]) return null;
  return { motivo: 'construccion', etiqueta: 'control', lectura: POR_CONSTRUCCION[v],
    ...(FEED.sinVeredicto() && PUBLICABLES_POR_CONSTRUCCION.has(v) ? { publica: true } : {}) };
}

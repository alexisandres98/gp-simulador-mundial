...
- `GET /api/tt/{board?days,agenda,match?id,live?id&ga&gb&i&j&server,players?q&gender,player?id,ranking?gender,tournaments,tournament?id,sim?a&b&best_of&first,track,model,competitions,brief,read?id,search?q}` — **(admin; `GP_TT_PUBLIC_ENABLED` abre)** tenis de mesa (8-sep, blueprint 9.0). `sim/read/brief/track` son Pro; las tesis viajan dentro de `board`/`match`. Sonda: `/api/internal/tt?key=[&odds=1&rec=1&settle=1&board=1&selftest=1&tail=1&n=N&history=N]`.
...
| Tenis de mesa (`ttopps`, `ttgames`, `ttmatch/<evento:código>`, `ttplayers`, `ttplayer/<id>`, `ttrank`, `tttours`, `tttour/<id>`, `ttsim`, `ttmap`, `ttperf`, `ttbrief`, `ttask`, `ttmodel`) — terminal `/x`, admin-only | ✅ Nueva (8-sep) | Oportunidades con la card de la casa (todo en sombra), agenda WTT con vivo (games + puntos + servidor), cockpit del partido con el Visual OS del blueprint 9.0 (Service Braid, Game State Lattice, Deuce Return Loop, Correct Score Foldout, Game Script River, Point-Margin Fold, Leverage Spine, Market Implied Process, Payoff Equivalence Weave, Evidence Lens), mercados de tres casas, forma, h2h, registro de invalidación; jugadores por género y fichas (el punto contra el circuito, Form Ribbon), ranking GP vs WTT, eventos con cuadro y título, mapa de integridad de competiciones, simulador BO5/BO7, sombra, brief, Pregúntale a GP, motor. |

...
function buildDto(scan, { resolveTeamId = () => null, now = Date.now(), maxItems = 60 } = {}) {
  ...
  const price_lag = lagAll.filter(l => l.is_soft).slice(0, maxItems).map(l => ({
    ...
    is_soft: true, is_exchange: !!l.is_exchange, venue_kind: l.venue_kind || 'sportsbook',
    is_favorite: !!l.is_favorite, favorite_outcome: l.favorite_outcome || null, favorite_prob: l.favorite_prob != null ? l.favorite_prob : null,
    source_role: l.source_role || null,
